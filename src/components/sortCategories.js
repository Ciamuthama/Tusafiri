...
const [activeSort, setActiveSort] = useState('Popular')
  ...
  return (
    <View className='flex-row justify-around items-center mx-4 bg-neutral-300 shadow-sm rounded-xl p-2 px-4 space-x-2'>
          {
              sortCategories.map((sort, index) => {
                  let isActive = sort === activeSort;
                  let activeButtonClass = isActive ? 'bg-white shadow-sm': ''
                  return (
